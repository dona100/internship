...
def get_employee_permissions(request):
    try:
        # Replace YourModel with the model you want to get default permissions for
...
class TimeCycleTaskAllocation(APIView):
    ...
    def delete(self, request, time_cycle_id):
        try:
            time_cycle = TimeCycle.objects.get(id=time_cycle_id)
            Allocation.objects.filter(time_cycle=time_cycle).delete()
            # Optionally, you might also want to delete the time cycle itself
            # time_cycle.delete()
        ...
        try:
            ...
            for employee in employees:
                ...
                employee_data = {
                    'employee': {
                        'id': employee.id,
                        'name': employee.employee_name,  # Replace with actual employee name field
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            otp = str(uuid.uuid4().int)[:6]  # Generate a 6-digit OTP
...
class ResetPasswordView(APIView):
    ...
    def post(self, request):
        ...
        print(f"Stored OTP: {stored_otp}, Attempted OTP: {otp_attempt}")
        ...
        if stored_otp and stored_otp == otp_attempt:
            ...
            try:
                ...
                # Optionally, you may want to invalidate existing sessions here

                # Delete the OTP from the database
                OTP.objects.filter(user=user, otp=otp_attempt).delete()
